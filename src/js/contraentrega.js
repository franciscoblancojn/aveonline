function init_WC_contraentrega(load = true) {
    ...
    WC_contraentrega.rebug = function () {
        ...
        if (this.checked) {
            e = document.getElementById(active.id.split("wc_contraentrega_off").join("wc_contraentrega_on"))
        } else {
            e = document.getElementById(active.id.split("wc_contraentrega_on").join("wc_contraentrega_off"))
        }
        if (!e.checked) {
            e.click()
        }
...
window.onload = function () {
    ...
    const panoptico = new MutationObserver((mutaciones, observer) => init_WC_contraentrega(false));
